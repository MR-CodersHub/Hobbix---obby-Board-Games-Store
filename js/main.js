...
const AppState = {
  ...
  updateRTLUI() {
    const btns = document.querySelectorAll('.rtl-toggle-btn, #rtl-toggle-btn, [onclick*="toggleRTL()"]');
    btns.forEach(btn => {
...
function openQuickView(productId) {
  ...
  const modeText = prod.mode || 'Co-op & Competitive';
  const features = Array.isArray(prod.features) ? prod.features : [];
  const specialText = prod.special || prod.description;
  ...
  body.innerHTML = `
    <div class="quickview-modal-layout">
      ...
      <div class="quickview-img-box">
        ...
        ${prod.badge ? `<span class="quickview-badge-overlay"><i class="fa-solid fa-trophy"></i> ${prod.badge}</span>` : ''}
        ${prod.discount ? `<span class="card-badge-discount quickview-discount-tag">${prod.discount} OFF</span>` : ''}
      </div>

      <!-- Right Column: Organized Game Details -->
      <div class="quickview-content-box">
        <div class="quickview-tags-row">
          <span class="badge badge-gold"><i class="fa-solid fa-tag"></i> ${prod.category}</span>
          <span class="badge badge-gold"><i class="fa-solid fa-gamepad"></i> ${modeText}</span>
          <span class="badge badge-gold"><i class="fa-solid fa-circle-check"></i> ${prod.inStock !== false ? 'In Stock (Seattle Vault)' : 'Backorder'}</span>
        </div>

        <h2 class="quickview-modal-title">${prod.title}</h2>

        <div class="quickview-meta-row">
          <div class="quickview-rating">
            <i class="fa-solid fa-star text-gold"></i>
            <span>${prod.rating}</span>
            <span class="quickview-reviews-count">(${prod.reviews || 120}+ Verified Reviews)</span>
          </div>
        </div>
        ...
        <div class="quickview-price-row">
          ...
          ${prod.discount ? `<span class="badge badge-gold" style="font-size: 0.72rem; padding: 0.2rem 0.55rem;">Save ${prod.discount}</span>` : ''}
        </div>

        <!-- Section 1: Full Game Overview -->
        <div class="quickview-section">
          <h4 class="quickview-section-title"><i class="fa-solid fa-book-open text-gold"></i> Game Overview</h4>
          <p class="quickview-description">${prod.description}</p>
        </div>

        <!-- Section 2: Tabletop Specifications Grid -->
        <div class="quickview-section">
          <h4 class="quickview-section-title"><i class="fa-solid fa-sliders text-gold"></i> Tabletop Specifications</h4>
          <div class="quickview-specs-grid">
            <div class="quickview-spec-card">
              <i class="fa-solid fa-users text-gold"></i>
              <div class="quickview-spec-info">
                <span class="spec-label">Players</span>
                <strong>${prod.players}</strong>
              </div>
            </div>
            <div class="quickview-spec-card">
            ...
            <div class="quickview-spec-card">
              ...
              <div class="quickview-spec-info">
                ...
                <strong>${prod.difficulty}</strong>
              </div>
            </div>
            <div class="quickview-spec-card">
              <i class="fa-solid fa-child text-gold"></i>
              <div class="quickview-spec-info">
                <span class="spec-label">Age Rating</span>
                <strong>${prod.age ? `Ages ${prod.age}` : 'Ages 12+'}</strong>
              </div>
            </div>
          </div>
        </div>

        <!-- Section 3: What Makes It Special Highlight -->
        ${specialText ? `
          <div class="quickview-section">
            <div class="quickview-special-box">
              <div class="quickview-special-heading">
                <i class="fa-solid fa-gem text-gold"></i>
                <span>What Makes It Special</span>
              </div>
              <p class="quickview-special-body">${specialText}</p>
            </div>
          </div>
        ` : ''}

        <!-- Section 4: Complete Features List -->
        ${features.length > 0 ? `
          <div class="quickview-section">
            <h4 class="quickview-section-title"><i class="fa-solid fa-list-check text-gold"></i> Key Mechanics & Features</h4>
            <div class="quickview-features-pills">
              ${features.map(f => `<span class="quickview-feature-pill"><i class="fa-solid fa-check text-gold"></i> ${f}</span>`).join('')}
            </div>
          </div>
        ` : ''}
        ...
        <div class="quickview-actions-row">
          <button class="btn btn-primary quickview-btn-add" onclick="AppState.addToCart('${prod.id}'); closeQuickView();">
            <i class="fa-solid fa-cart-shopping"></i> Add To Cart — $${prod.price.toFixed(2)}
          </button>
          <button class="btn btn-secondary quickview-btn-wish ${isWishlisted ? 'active' : ''}" onclick="AppState.toggleWishlist('${prod.id}'); this.classList.toggle('active');" title="Save to Wishlist">
        ...
        <!-- Trust Perks Strip -->
        <div class="quickview-perks-bar">
          <span><i class="fa-solid fa-truck-fast text-gold"></i> Same-Day Dispatch</span>
          <span><i class="fa-solid fa-shield-halved text-gold"></i> Mint-Box Guarantee</span>
          <span><i class="fa-solid fa-rotate-left text-gold"></i> 30-Day Returns</span>
        </div>
      </div>
    </div>
  `;
...
// Generate Standard Product Card HTML (Clean, Simplified & Scannable)
function createProductCardHTML(prod, matchScore = null) {
  const isWishlisted = AppState.wishlist.includes(prod.id);
  const modeText = prod.mode || 'Co-op & Competitive';
  const topFeature = Array.isArray(prod.features) && prod.features.length > 0 ? prod.features[0] : null;
  const matchPct = matchScore || prod.matchPct;
    ...
    <div class="game-card" data-product-id="${prod.id}">
      <div class="card-media">
        ...
        <div class="card-badges">
          ${matchPct ? `<span class="card-badge-glow"><i class="fa-solid fa-wand-magic-sparkles"></i> ${matchPct}% Match</span>` : ''}
          ${prod.badge && !matchPct ? `<span class="card-badge-glow"><i class="fa-solid fa-sparkles"></i> ${prod.badge}</span>` : ''}
      ...
      <div class="card-body-content">
        ...
        <div class="card-key-highlights">
          <span class="card-highlight-badge"><i class="fa-solid fa-gamepad text-gold"></i> ${modeText}</span>
          ${topFeature ? `<span class="card-highlight-badge"><i class="fa-solid fa-gem text-gold"></i> ${topFeature}</span>` : ''}
        </div>
        ...
        <div class="card-footer-row">
          ...
          <div class="card-actions-group">
            <button class="btn-card-quickview" onclick="openQuickView('${prod.id}')" title="View Full Game Details">
  ...
  return `
    <div class="event-card">
      ...
      <div class="event-content">
        <span class="event-type-tag"><i class="fa-solid fa-dice-d20"></i> <span>${evt.game}</span></span>
        <h3 class="event-title">${evt.title}</h3>
        <ul class="event-details-list">
          <li><i class="fa-regular fa-clock"></i> <span>${evt.time}</span></li>
          <li><i class="fa-solid fa-user-shield"></i> <span>Host: ${evt.host}</span></li>
          <li><i class="fa-solid fa-ticket"></i> <span>Entry: ${evt.fee}</span></li>
          <li><i class="fa-solid fa-location-dot"></i> <span>${evt.location}</span></li>
        </ul>
        ...
        <button class="btn btn-secondary btn-sm" onclick="openJoinEventModal('${evt.id}')">
